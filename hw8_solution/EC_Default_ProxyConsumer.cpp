...
//    printf("worker thread: working on an event from EDFQ...\n");
    ...
    TAO_EC_HW6_Worker hw6_worker (std::get<0>(item));
    pxy->event_channel_->for_each_consumer (&hw6_worker);
  }
  return NULL;
}

OurSingletonWorker::
    OurSingletonWorker (TAO_EC_Event_Channel_Base* ec)
  : event_channel_ (ec)
{
  ...
  : TAO_EC_ProxyPushConsumer (ec)
{
  ...
  workerObj = OurSingletonWorker::getInstance(ec);
